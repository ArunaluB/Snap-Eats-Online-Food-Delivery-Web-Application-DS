...
//           }
//         } else {
//           setError("Use this button to navigate to your order...");
//           setError
...
//         // setOrderStatus(details as OrderStatusResponse);
...
export const OrderSuccessPage = () => {
  ...
  const navigate = useNavigate();
  const [orderDetails, setOrderDetails] = useState<OrderDetails | null>(null);
  const [restaurantDetails, setRestaurantDetails] = useState<RestaurantDTO | null>(null);
  const [userAddress, setUserAddress] = useState<AddressDTO | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [isExpanded, setIsExpanded] = useState(false);
  ...
  useEffect(() => {
    const restoreAndPlaceOrder = async () => {
      ...
      if (!details) {
        ...
        if (storedOrderDetails) {
          ...
          setError("Use this button to navgate your order.");
          // toast.error("No order details found. Please check your latest order.", { duration: 3000 });
          setLoading(false);
          return;
        }
      ...
      try {
        ...
        console.log("Placing order with userId:", userId, "and details:", details);
        const response = await axios.post(`http://localhost:8222/order-service/api/orders`, {
          ...details,
          orderId: `order-${userId}-${Date.now()}`,
        });
        ...
        toast.error(`Unable to place order: ${err.message}`, { duration: 3000 });
        hasPlacedOrder.current = false;
        setOrderPlaced(false);
      } finally {
        setLoading(false);
    ...
    restoreAndPlaceOrder();
  }, [orderDetails, orderPlaced, userId]);

  // Fetch user address and restaurant details
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [userResponse, restaurantResponse] = await Promise.all([
          ...
          orderStatus?.restaurantId
            ? axios.get(`http://localhost:8222/restaurant-service/api/restaurants/${orderStatus.restaurantId}`).catch((err) => {
        ...
        setError(`Unable to load order details: ${err.message}`);
        toast.error(`Unable to load order details: ${err.message}`, { duration: 3000 });
      }
    };

    if (orderStatus?.restaurantId && userId) {
      fetchData();
    }
  }, [userId, orderStatus]);
  ...
  useEffect(() => {
    const fetchOrderStatus = async () => {
      ...
      try {
        ...
        setStatusError(`Unable to fetch order status: ${err.response?.status === 404 ? "No recent order found." : err.message}`);
        toast.error(`Unable to fetch order status: ${err.response?.status === 404 ? "No recent order found." : err.message}`, { duration: 3000 });
      } finally {
        setStatusLoading(false);
      }
    };
        ...
        <div className="lg:w-2/3 space-y-6">
          <div className="bg-white rounded-lg p-6 shadow-md">
            ...
            {statusLoading ? (
              ...
              <p className="text-gray-500">No recent order found. Please check your order history.</p>
            )}
          </div>
          <div className="bg-white rounded-lg p-6 shadow-md">
            <h2 className="font-bold text-xl text-gray-800 mb-4">Delivery Details</h2>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            {restaurantDetails && orderStatus ? (
              <div className="flex items-center gap-4">
                ...
                />
                <div>
                  <p className="font-bold text-gray-800">{restaurantDetails.name || 'Unknown Restaurant'}</p>
                  <p className="text-sm text-gray-500">{getFormattedAddress(restaurantDetails.address) || 'Address not available'}</p>
                </div>
              </div>
            ) : (
              <p className="text-gray-500">Restaurant data not available. Please check your order details.</p>
            )}
          </div>
          {orderStatus ? (
            <div className="bg-white rounded-lg shadow-md p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <p className="font-bold text-xl">Order Summary ({orderStatus.items.length} items)</p>
                </div>
                <button
                  onClick={toggleExpanded}
                  className={`text-gray-500 hover:text-gray-700 transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`}
                  ...
                  {orderStatus.items.map((item, index) => (
                    <div key={index} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-gray-800">{item.itemName || 'Unknown Item'}</p>
                          {item.customizations.length > 0 && (
                            <p className="text-sm text-gray-500">
                              Customizations: {item.customizations.join(', ')}
                            </p>
                          )}
                          <p className="text-sm text-gray-500">LKR {(item.totalPrice || 0).toFixed(2)}</p>
                        </div>
                      </div>
                      <span className="font-medium text-gray-800">{item.quantity || 0}</span>
                    </div>
                  ))}
                </div>
              )}
              <div className="mt-6 pt-4 border-t border-gray-200">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-gray-600">
                    <p>Subtotal</p>
                    <p>LKR {(orderStatus.items.reduce((sum, item) => sum + (item.totalPrice || 0), 0)).toFixed(2)}</p>
                  </div>
                  <div className="flex justify-between text-gray-600">
                    <p>Delivery Fee</p>
                    <p>LKR {(orderStatus.deliveryFee || 0).toFixed(2)}</p>
                  </div>
                  <div className="flex justify-between text-gray-600">
                    <p>Taxes & Other Fees</p>
                    <p>LKR {(orderStatus.taxAmount || 0).toFixed(2)}</p>
                  </div>
                  <div className="flex justify-between font-bold text-gray-800 pt-2 border-t border-gray-200">
                    <p>Total</p>
                    <p>LKR {((orderStatus.items.reduce((sum, item) => sum + (item.totalPrice || 0), 0) + (orderStatus.deliveryFee || 0) + (orderStatus.taxAmount || 0)).toFixed(2))}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md text-gray-500">
              No order data available. Please check your order history.
            </div>
          )}
        </div>
      </div>
