  ...
  useEffect(() => {
    const fetchLatestOrder = async () => {
      ...
      try {
        ...
        // Check for specific "no orders" error (e.g., HTTP 404)
        if (err.response?.status === 404 || err.response?.data?.message?.includes("No orders found")) {
          setOrderStatus(null); // No pending orders
        } else {
          setError(`Failed to load latest order: ${err.message}`);
          toast.error(`Failed to load latest order: ${err.message}`, { duration: 3000 });
        }
      } finally {
        setLoading(false);
      }
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-medium text-gray-800">
                      {getFormattedAddress(orderStatus.deliveryAddress)}
                    </p>
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-xl font-semibold text-gray-800">
                    Order Summary ({orderStatus.items.length} items)
                  </h2>
                </div>
                <button
