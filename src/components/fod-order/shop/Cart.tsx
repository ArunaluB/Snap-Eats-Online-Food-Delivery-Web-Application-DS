...
export const Cart = ({ userId, onClose }: CartProps): JSX.Element => {
    ...
    useEffect(() => {
        const fetchCartData = async () => {
            ...
            try {
                ...
                console.error('Error fetching cart data:', error);
                setCart(null); // Treat errors as no cart for simplicity
            } finally {
                setLoading(false);
            }
        };
    ...
    // Calculate totals
    const calculateTotals = () => {
        if (!cart || cart.items.length === 0) return { subtotal: 0, totalDiscount: 0, netTotalPrice: 0 };
            ...
            <div className="fixed top-0 right-0 h-full lg:min-w-[450px] bg-white shadow-lg z-50 transition-transform duration-300 ease-in-out transform translate-x-0 flex flex-col">
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        <div className="flex-1 overflow-y-auto">
                            <div className="px-4 py-3">
                                ...
                                <div className="bg-gray-50 rounded-lg p-4 shadow-sm space-y-3">
                                    <div className="flex justify-between text-gray-700">
                                        <span>Subtotal</span>
                                        <span>{calculateTotals().subtotal.toFixed(2)} LKR</span>
                                    </div>
                                    <div className="flex justify-between text-blue-600 font-medium">
                                        <span>Discount</span>
                                        <span>-{calculateTotals().totalDiscount.toFixed(2)} LKR</span>
                                    </div>
                                    <div className="flex justify-between text-black text-lg font-bold border-t pt-3">
                                        <span>Net Total</span>
                                        <span>{calculateTotals().netTotalPrice.toFixed(2)} LKR</span>
                                    </div>
                                    <div className="flex justify-between items-center text-sm text-gray-500">
                                        <span>Estimated Preparation Time</span>
                                        <span className="flex items-center gap-1">
                                            <span className="material-symbols-outlined text-base text-gray-400">schedule</span>
                                            {getLongestPreparationTime()} min
                                        </span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="px-4 py-4 border-t">
                            <button
                                className="w-full bg-black text-white py-3 rounded-lg font-bold hover:bg-gray-900 transform hover:scale-[1.01] transition-all"
                                onClick={() => window.location.href = `/fod-order/order-summary/${userId}`}
                            >
                                Go to checkout
                            </button>
                        </div>
